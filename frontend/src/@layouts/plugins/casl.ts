...
export const canNavigate = (to: RouteLocationNormalized) => {
  try {
    ...
    // If route doesn't have action/subject in meta, allow navigation (public route)
    ...
    return to.matched.every(route => {
      // If route has no action/subject, it's public - allow navigation
      if (!route.meta.action && !route.meta.subject)
        return true

      // Check ability for routes with action/subject
      try {
        return ability.can(route.meta.action, route.meta.subject)
      }
      catch {
        // If ability check fails, allow navigation (fail open)
        return true
      }
    })
  }
  catch {
    // If ability is not initialized, allow navigation (fail open)
    return true
  }
}
